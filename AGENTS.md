...
```bash
cd ai-ml/02-ml-core/01-linear-regression/
python notebook.py              # saves visualization as .png
jupytext --sync notebook.ipynb  # sync .py ↔ .ipynb after editing either
```
...
### Standard 4-File Pattern (MANDATORY)
...
### AI/ML 5th File: notebook.py (paired with notebook.ipynb)

AI/ML topics include a `notebook.py` paired with `notebook.ipynb` via [Jupytext](https://jupytext.readthedocs.io/). The `.py` runs in any terminal; the `.ipynb` opens in Jupyter/Colab. They stay in sync:

```bash
jupytext --set-formats ipynb,py:percent notebook.ipynb  # first-time pair
jupytext --sync notebook.ipynb                          # sync after editing
```
...
7. AI/ML topics: `notebook.py` paired with `notebook.ipynb` via jupytext
